...
import { getCourses, getAllChatMessages, approveMessage as approveMsg, deleteMessage } from "../lib/supabase"
...
export const ChatModerationPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: messagesData, error: messagesError } = await getAllChatMessages()
  ...
  const handleApproveMessage = async (id: string) => {
    try {
      const { error } = await approveMsg(id)

      if (error) throw error

      setMessages(messages.map((message) => (message.id === id ? { ...message, is_moderated: true } : message)))
    } catch (err) {
      console.error("Ошибка при одобрении сообщения:", err)
      setError("Не удалось одобрить сообщение")
    }
  }
    ...
    <Layout>
      <div className="p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Проверка и управление сообщениями в чатах курсов</p>
        </div>
            ...
            {filteredMessages.map((message) => {
              ...
              return (
                <div key={message.id} className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                  <div
                    className={`px-4 py-2 flex justify-between items-center ${needsModeration ? "bg-yellow-50" : "bg-gray-50"}`}
                  >
                  ...
                  {needsModeration && (
                    <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex justify-end space-x-2">
                      ...
                      <Button size="sm" onClick={() => handleApproveMessage(message.id)}>
